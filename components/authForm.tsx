  ...
  return (
    <form className='flex flex-col gap-3 items-center' onSubmit={handleSubmit}>
      <div className="mb-4 flex gap-1 w-full flex-col">
        ...
        <input type="email" id="email" name="email" required className="px-2 py-2 mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 border-1" value={form.email} onChange={handleChange} />
      </div>
      <div className="mb-4 flex gap-1 w-full flex-col">
        <label htmlFor="password" className="block text-sm font-medium dark:text-gray-200 text-gray-800">Password</label>
        <input type="password" id="password" name="password" required className="mt-1 px-2 py-2 block w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 border-1" value={form.password} onChange={handleChange} />
      </div>
      {error && (
